...
function getCookie(){var results=document.cookie.match(new RegExp("f"+forumId+"style=(.*?)(?:;|$)"));if(results!==null&&results.length>1){return results[1];}}
function setCookie(cssStyleKey){var cookie="f"+forumId+"style="+cssStyleKey+"; expires=",date=new Date(),offset=1000*60*60*24*365;if(!cssStyleKey){offset=-offset;}
date.setTime(date.getTime()+offset);cookie+=date.toGMTString();document.cookie=cookie;}
return{changeTheme:function(themeName){var theme,shouldRevert;theme=this.themes[themeName];if(usingDynamicTheme){removeTheme();themeHeader.src=defaultThemeHeaderSrc;}
if(theme){if(theme.cssHref){addTheme(theme.cssHref);setCookie(themeName);usingDynamicTheme=true;}
if(theme.headerSrc){themeHeader.src=theme.headerSrc;}else{themeHeader.src=defaultThemeHeaderSrc;}
if(!theme.cssHref&&!theme.headerSrc){shouldRevert=true;}}else{shouldRevert=true;}
if(shouldRevert){setCookie("");usingDynamicTheme=false;}},initialize:function(selector){var themeName,options,optionCount,option,cssLocation,headerImage,key,themes,theme,firstThemeName;themeHeader=document.getElementById("theme_header");defaultThemeHeaderSrc=themeHeader.src;selectorElement=selector;themeName=getCookie();themes={};options=selectorElement.options;optionCount=options.length;for(var i=0;i<optionCount;++i){option=options[i];key=option.value;if(i===0){firstThemeName=key;}
